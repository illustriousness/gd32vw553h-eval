...
def with_metaclass(meta, *bases):
    class temporary_class(*bases, metaclass=meta):
        pass

    return temporary_class


PROJECT_ROOT = dirname(dirname(abspath(__file__)))

# Default wrapper configuration. Running `python tools/memmap.py` uses these.
...
# auto: color only when stdout is a terminal; always: force; never: disable.
COLOR = "auto"
...
def _choice_type(case_func, prefer_hyphen):
    def middle(choices, type_name):
        normalized_choices = [str(choice) for choice in choices]

        def parse_type(value):
            value = str(value)
            normalized = case_func(value)
            if prefer_hyphen:
            ...
            raise argparse.ArgumentTypeError(
                "{0} is not a supported {1}. Supported {1}s are: {2}".format(
                    value,
                    type_name,
...
argparse_uppercase_type = _choice_type(str.upper, False)
argparse_lowercase_hyphen_type = _choice_type(str.lower, True)


def argparse_filestring_type(value):
    if exists(value):
        return value
    raise argparse.ArgumentTypeError(
        "{0} does not exist in the filesystem.".format(value)
    )
...
def _project_path(path_value):
    ...
    return normpath(join(PROJECT_ROOT, path_value))
...
def _colorize_deltas(text, mode):
    if not _color_enabled(mode):
        return text
    ...
    def replace(match):
        ...
        if value > 0:
            ...
            color = ANSI_GRAY
        return color + match.group(0) + ANSI_RESET

    return DELTA_RE.sub(replace, text)


def _load_map_view(map_file):
    map_view_path = map_file + ".view"
    if not exists(map_view_path):
        return

    global TOOLCHAIN_BINDIR, TOOLCHAIN_ROOTDIR
    with open(map_view_path, "r", encoding="utf-8") as file_desc:
        map_view_data = yaml.safe_load(file_desc) or {}
    compiler_path = map_view_data.get("compilerPath")
    if compiler_path:
        TOOLCHAIN_BINDIR = dirname(compiler_path)
        TOOLCHAIN_ROOTDIR = dirname(TOOLCHAIN_BINDIR)


def _run_memap(map_file, toolchain, depth, export_format, output_file=None):
    memap = MemapParser()
    _load_map_view(map_file)

    if memap.parse(map_file, toolchain) is False:
        return None, False

    output = memap.generate_output(export_format, depth, output_file)
    if output is False:
        return None, False
...
def _ensure_old_map(map_file):
    old_map_file = map_file + ".old"
    if exists(old_map_file):
        return old_map_file, False
    shutil.copy2(map_file, old_map_file)
    return old_map_file, True


def _update_old_map(map_file):
    shutil.copy2(map_file, map_file + ".old")


def run_configured():
    map_file = _project_path(MAP_FILE)
    output_file = _project_path(OUTPUT_FILE)

    if not exists(map_file):
        print("Map file not found: {}".format(map_file), file=stderr)
        return 1

    try:
        _ensure_old_map(map_file)
        output, ok = _run_memap(map_file, TOOLCHAIN, DEPTH, EXPORT, output_file)
        if not ok:
            return 1

        if EXPORT == "table" and output:
            print(_colorize_deltas(output, COLOR))

        _update_old_map(map_file)
        return 0
    except OSError as error:
        print("I/O error: {}".format(error), file=stderr)
        return 1
